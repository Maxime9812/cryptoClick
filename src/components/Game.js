...
import anga from '../img/anga.jpeg'
...
export default class Game extends Component {
	...
	changeBackground(){
		switch(this.state.level){
			case 1: this.backGround = img1
			break
			case 2: this.backGround = img2
			break
			case 3: this.backGround = img3
			break
			case 4: this.backGround = img4
			break
			case 5: this.backGround = img5
			break
			case 6: this.backGround = img6
			break
			case 7: this.backGround = anga
			break
		}
	}
	...
	updateCoin(){
		this.setState({coin: this.state.coin+this.state.ppc,sound: Sound.status.PLAYING})
	}
	stopSound(){
	...
	upgrade(price,nbP,id,type){
		...
		switch(type){
			case "ppc": this.state.upgradesPPC[id] = this.state.upgradesPPC[id]+ 1;
						this.setState({coin: this.state.coin-price,ppc: this.state.ppc+ parseInt(nbP),sound:Sound.status.PLAYING})
			break;
			case "pps": this.state.upgradesPPS[id] = this.state.upgradesPPS[id]+ 1;
						this.setState({coin: this.state.coin-price,pps: this.state.pps+parseInt(nbP),sound:Sound.status.PLAYING})
  ...
  render() {
  	switch(this.state.page){
  		case "pcUpgrade": this.changeBackground()
  					return (
			      <div className="App">
			      <Sound url={buySound} playStatus={this.state.sound} onFinishedPlaying={this.stopSound} volume={30} muted={false}/>
  		...
  		case "miner":
			      ...
			      <Sound url={mineSound} playStatus={this.state.sound} onFinishedPlaying={this.stopSound} volume={100} muted={false}/>
			      <img id="imgBackground" src={this.backGround}/>
  		...
  		case "settings":
			      ...
			      <a className="pauseText" onClick={this.startTimer}>Resume</a>
			      <a className="pauseText" onClick={this.props.returnHome}>Quit game</a>
			      </div>
		    );
  		break;
  		case "":
